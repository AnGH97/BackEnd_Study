...
public class HW1_new_ver {
	
	static void computer(int[] com) {
	...
	static void comNum(int[] com) {
		computer(com);
		
		while(true) {
			boolean ch = numCheck(com);
			if(ch == true) {
				break;
			}
			else if(ch == false) {
	...
	static void gameCheck(int c[], int m[], int answer[]) {
		//out	ball	strike	
		boolean a = true;
	...
	static void gameStart() {
		...
		while(true) {
			...
			myNum(my);
			gameCheck(com, my, answer);

			if(answer[2] == 3) {
				System.out.println("!!홈런!!");
				System.out.println("3 strike로 게임을 종료합니다.");
				return;
			}
			else {
				System.out.println(answer[0] + "out, "+answer[1] + "ball, " + answer[2] + "strike 입니다.");
			}
		}
	}
